function [cost,grad] = Tensor3DNeuralNetworkRegressionCost(theta, layerSize, lambda, sparsityParam, beta, data, Y, linear)
...
X{1} = data; % The input is X_1. X{1}=X
...
for i=1:L %times of information transformation
    ...
    if i+1 <= L
       rho = mean(X{i+1},4); %average activation value for sparsity
...
cost = cost + 0.5 * sum(sum(sum(sum((X{L+1} - Y).^2)))) / size(data,4);
%Test_err = [];
%Test_err = [Test_err, cost/(size(Ytrain,1)*size(Ytrain,2)*size(Ytrain,3))];
