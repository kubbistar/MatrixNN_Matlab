...
Xtrain = squeeze(X(:,:,:,1,1,1:400));
%Xtest = squeeze(X(:,:,1,1,1,401:433)); 
Xtest = squeeze(X(:,:,:,1,1,401:433)); 

% Ytrain = squeeze(Y(:,:,1,1:400));
% Ytest = squeeze(Y(:,:,1,401:433));
Ytrain = squeeze(Y(:,:,:,1:400));
Ytest = squeeze(Y(:,:,:,401:433));
...
for i = 1:length(lambdas)   
    ...
    %layerSize{3}.I = 40; layerSize{3}.J = 40; % the size of hidden layer: new added
...
%     layerSize{4}.I = 25; layerSize{4}.J = 25; % the size of Output layer
    ...
    for i=1:L
        ...
        if (i+1>L)
            ...
            XX{i+1} = 1./(1+exp(-N{i})); % Acting also can test on tangent function
        end    
    end;
...
[best_lambda_testerror,idx] = min(Test_err)   %--to find the smallest value and the position or index
...
% general, beta is not sensitive, I may suggest you fix beta =2.5 or 3.
