function [cost,grad] = Tensor3DRegressionCost(theta, layerSize, lambda, sparsityParam, beta, data, Y, linear)
...
%     grad.V{i-1} = zeros(size(V{i-1}));
    ...
    B{i-1} = reshape(theta(count_start:count_end), layerSize{i}.I,layerSize{i}.J,layerSize{i}.K);
...
dimsX = size(data);
numData = dimsX(end); %543
XX{1} = data; % The input is X_1. X{1}=X
scale = size(Y,1)*size(Y,2)*size(Y,3)*size(Y,4); %25*25*4*543
%for i=1:L %times of information transformation
i=1;
    UU = {U{i}, V{i}, W{i}};
    XX{L+1} = double(ttm(tensor(XX{i}), UU, (1:length(UU))) + ttm(tensor(B{i}, [size(B{i}) 1]), ones(1,numData)', length(dimsX)));  
...
%     end
    cost = cost + 0.5*lambda*(norm(U{i},'fro')^2 + norm(V{i},'fro')^2 + norm(W{i},'fro')^2) ;
    if i+1 <= L
       rho = mean(XX{i+1},4); %average activation value for sparsity
...
cost = cost + 0.5 * sum(sum(sum(sum((XX{L+1} - Y).^2)))) / size(data,4);
...
if linear
    ...
    delta{L+1} = - (Y - XX{L+1} ).* XX{L+1} .* (1 - XX{L+1});
end
...
i=1;
    UU = {U{i}, V{i}, W{i}}; 
    if i>1
        delta{i} = (bsxfun(@plus, double(ttm(tensor(delta{i+1}), UU, (1:length(UU)), 't')), part_delta{i})) .* XX{i} .* (1 - XX{i});
        %delta{i} = (bsxfun(@plus, prod_mat_tensor_mat(U{i}', delta{i+1}, V{i}'), part_delta{i})).* X{i} .* (1 - X{i});
    end;
    Ud = UU;
    Ud{1} = eye(size(UU{1},2));
    tmpXd = ttm(tensor(XX{i}), Ud, [1:3]);
    gradU = double(ttt(tensor(delta{i+1}), tmpXd, [2 3 4], [2 3 4]))/size(data,4)  + lambda * U{i};
    Ud = UU;
    Ud{2} = eye(size(UU{2},2));
    tmpXd = ttm(tensor(XX{i}), Ud, [1:3]);
    gradV = double(ttt(tensor(delta{i+1}), tmpXd, [1 3 4], [1 3 4]))/size(data,4) + lambda * V{i};
    Ud = UU;
    Ud{3} = eye(size(UU{3},2));
    tmpXd = ttm(tensor(XX{i}), Ud, [1:3]);
    gradW = double(ttt(tensor(delta{i+1}), tmpXd, [1 2 4], [1 2 4]))/size(data,4) + lambda * W{i};
    ...
    gradB = mean(delta{i+1},4);
    %gradB = sum(delta{i+1},4)/scale;
    grad = [gradU(:);gradV(:);gradW(:);gradB(:);grad];
%end;
