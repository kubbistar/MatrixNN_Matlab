function [U, V, W, B] = demo_Relation_RegRegression_ver3(sparsityParam,lambda,beta,maxIters)
% Version 1.000
%
...
layerSize{2}.I = 25; layerSize{2}.J = 25; layerSize{2}.K = 4;% the size of Output layer
...
%In simulation study:Lsim3D.mat for linear case; NLsim3D.mat for nonlinear case
Y(isnan(Y)) = 0;
...
[cost, grad] = Tensor3DRegressionCost(theta, layerSize, lambda, sparsityParam, beta, patches.Xtrain, patches.Ytrain, 'linear');
...
[opttheta, cost,~,info] = minFunc( @(p) Tensor3DRegressionCost(p, ...
...
i=1;
    UU = {U{i}, V{i}, W{i}};
    XX{L+1} = double(ttm(tensor(XX{i}), UU, (1:length(UU))) + ttm(tensor(B{i}, [size(B{i}) 1]), ones(1,size(Ytrain,4))', 4));  
...
% error.
[test_cost, ~] = Tensor3DRegressionCost(opttheta, layerSize, lambda, sparsityParam, beta, patches.Xtest, patches.Ytest, 'linear');
